...
%	\rule{\textwidth}{2pt}
	...
	\Input{Number of Folds: \(K = 4\) \newline
		Step Size: \(S_{size} = 5,000\): \newline
		Minimum Error Delta: \(\Delta_{min} = 10^{-6} \cdot\) meanResponse \newline
	...
	\While{ \textsf{\upshape NumberOfTrees \textless } \(M\) \textsf{\upshape and} \(S_{remaining}\) $\textsf{\upshape \textgreater \  0}$}
	{
		...
		\uIf{\(\Delta_{error} > \Delta_{min}\)}{\[S_{remaining} \leftarrow S_{pm}\]}
...
data from the 1995 FBI UCR." \cite{crimeCommunities} \todo{write something}
...
\begin{table}
	...
	\begin{tabular}{ | l || c |}
		...
		Number of CV Folds & 4 \\ \hline
		CV Step Size & 5000 \\ \hline
		\begin{tabular}{@{}l@{}}Number of CV Steps after \\ Falling Below Min Error Delta\end{tabular}
		& 3 \\ \hline
		Minimum Error Delta & 0.000001 $\cdot$ mean training response \\
		\hline
	\end{tabular}
...
\begin{table}
	\centering
		\begin{tabular}	{ | l || c |}
			...
			Min Examples In Node & 1, 50, 100, 250 \\ \hline
...
\todo{Crime communities citation request} 
